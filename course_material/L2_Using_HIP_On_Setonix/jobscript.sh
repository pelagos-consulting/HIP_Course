...
#SBATCH --gpu-bind=closest         # Choose the closest GPU for each task
...
srun -N $SLURM_JOB_NUM_NODES -n $SLURM_NTASKS -c $OMP_NUM_THREADS -m block:block:block ./hello_devices_mpi_onefile_hipcc.exe
