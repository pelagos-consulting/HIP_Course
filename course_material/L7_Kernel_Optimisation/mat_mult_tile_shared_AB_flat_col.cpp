...
__global__ void mat_mult_tile_shared_AB (
    ...
    // shared_A is of size (chunk_len, L0)
    // shared_B is of size (chunk_len, L1)
    ...
    if ((i0<N0_C) && (i1<N1_C)) {
        ...
        for (int chunk_id=0; chunk_id<nchunks; chunk_id++) {
            ...
            // Fill shared_A_star first
            for (int offset_S=w0; offset_S<L0*chunk_len; offset_S+=nthreads) {
                
                // Coordinates within shared_A_star of size (chunk_len, L0)
                int j0 = offset_S / L0;
                int j1 = offset_S % L0;
                
                // Get the offset into A of size (N0_C, N1_A_star)
                size_t offset_A = (blockIdx.y*blockDim.y+j1)*chunk_len*nchunks 
                    + chunk_id*chunk_len+j0;
                
                if (offset_A<N0_C*nchunks*chunk_len) {
            ...
            // Then fill shared_B_star
            for (int offset_S=w0; offset_S<L1*chunk_len; offset_S+=nthreads) {

                // Coordinates within shared_B_star of size (chunk_len, L1)
                int j0 = offset_S / L1;
                int j1 = offset_S % L1;
                ...
                if (offset_B<N1_C*nchunks*chunk_len) {
            ...
            for (size_t n=0; n<chunk_len; n++) {
                ...
                temp+=shared_A_star[n*L0+s0]*shared_B_star[n*L1+s1];
            }
