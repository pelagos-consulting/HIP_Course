...
__global__ void mat_mult_tile_shared_AB_vector (
    ...
    float_type* shared_A = (float_vector_type*)&shared[0];
    float_type* shared_B = (float_vector_type*)&shared[L0*chunk_len*sizeof(float_type)];
    ...
    for (int chunk_id=0; chunk_id<nchunks; chunk_id++) {
        ...
        // Get vector handles on shared memory
        ...
        // Loop over shared memory and compute dot product 
        ...
        for (size_t n=0; n<nvectors; n++) {
            ...
            temp.w += shared_A_v[n].w*shared_B_v[n].w;
#else
