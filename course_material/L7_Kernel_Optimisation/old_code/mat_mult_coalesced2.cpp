...
__global__ void mat_mult (
    ...
    // Position of the block within the Grid
    int pg = blockIdx.y*gridDim.x + blockIdx.x;
    
    // Map block id to new coordinates
    //int g0 = pg / gridDim.y;
    //int g1 = pg % gridDim.y; 
    
    // Position within the block
    int pb = threadIdx.y*blockDim.x + threadIdx.x;

    // Compute coordinates in C
    size_t i0 = blockIdx.x*blockDim.x + pb / blockDim.y;
    size_t i1 = blockIdx.y*blockDim.y + pb % blockDim.y;
